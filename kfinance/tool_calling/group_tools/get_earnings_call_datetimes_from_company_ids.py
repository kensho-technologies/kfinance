...
class GetEarningsCallDatetimesFromCompanyIds(KfinanceTool):
    ...
    def _run(self, company_ids: list[int]) -> dict[str, list[str] | str]:

        companies = Companies(kfinance_api_client=self.kfinance_client.kfinance_api_client,
                              company_ids=company_ids)

        fetched_results: dict[Company, list[datetime] | None] = companies.earnings_call_datetimes
        stringified_results = dict()
        for company, result in fetched_results.items():
            stringified_result = "unavailable" if result is None else [dt.isoformat() for dt in result]

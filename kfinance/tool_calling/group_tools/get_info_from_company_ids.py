...
class GetInfoFromCompanyIds(KfinanceTool):
    ...
    description: str = dedent("""
        Get the information associated with an company_id. Info includes company name, status, type, simple industry, number of employees, founding date, webpage, HQ address, HQ city, HQ zip code, HQ state, HQ country, and HQ country iso code.
    ...
    def _run(self, company_ids: list[int]) -> dict[str, str | dict]:
        ...
        companies = Companies(kfinance_api_client=self.kfinance_client.kfinance_api_client,
                              company_ids=company_ids)

        fetched_results: dict[Company, dict | None] = companies.info

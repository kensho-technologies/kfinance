...
class LangChainOpenAIChat:
    def __init__(self, tool_mode: ToolMode = ToolMode.GROUP) -> None:
        """"""
        ...
        agent = create_tool_calling_agent(
            llm=llm, tools=kfinance_client.get_langchain_tools(tool_mode=tool_mode), prompt=prompt
        )
        # Create an agent executor by passing in the agent and tools
        self.agent_executor = AgentExecutor(
            agent=agent,
            tools=kfinance_client.get_langchain_tools(tool_mode=tool_mode),
            verbose=True,
        )

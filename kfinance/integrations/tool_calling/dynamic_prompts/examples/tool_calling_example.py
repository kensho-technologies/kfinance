"""Complete example of tool calling with dynamic prompts."""
...
class EnhancedFinancialQueryProcessor:
    ...
    def process_query(self, query: str) -> Dict[str, Any]:
        ...
        if self.enable_dynamic_prompts and self.prompt_manager:
            ...
            logger.info("Found %d similar examples:", len(similar_examples))
            if similar_examples:
                for i, example in enumerate(similar_examples[:2], 1):
                    ...
                    logger.info("  %d. %s (similarity: %.3f)", i, example_query, similarity)
            else:
                logger.info("  No similar examples found")
        else:
            prompt = BASE_PROMPT
            stats = {"example_count": 0, "total_words": len(prompt.split())}
            logger.info("Using static prompt (dynamic prompts disabled)")

        # Step 2: Generate LLM response with tool calling
        logger.info("Generating LLM response with %d available tools...", len(self.available_tools))

        llm_response = self.llm_client.generate_with_tools(
            prompt=prompt, available_tools=self.available_tools
        )

        logger.info("  LLM reasoning: %s", llm_response.get("reasoning", "No reasoning provided"))
        ...
        if tool_calls:
            ...
            for i, tool_call in enumerate(tool_calls, 1):
                ...
                logger.info("  %d. %s(%s)", i, tool_name, parameters)
                ...
                if tool_result.get("success"):
                    ...
                    logger.info("    Success: %s = %s", line_item, value)
                else:
                    error = tool_result.get("error", "Unknown error")
                    logger.info("    Failed: %s", error)
        else:
            logger.info("No tool calls generated by LLM")
...
def demo_basic_usage() -> None:
    ...
    logger.info("Initialized processor with %d permissions", len(user_permissions))
    logger.info(
        "Dynamic prompts: %s", "Enabled" if processor.enable_dynamic_prompts else "Disabled"
    )
    ...
    for i, query in enumerate(test_queries, 1):
        logger.info("=" * 60)
        logger.info("TEST %d/%d", i, len(test_queries))
        logger.info("=" * 60)
        ...
        if result["success"]:
            ...
            successful_calls = [r for r in tool_results if r["result"].get("success")]
            logger.info(
                "Query processed successfully with %d successful tool calls", len(successful_calls)
            )
        else:
            logger.info("Query processing failed")

        # Show prompt stats
        stats = result["prompt_stats"]
...
def demo_static_vs_dynamic_comparison() -> None:
    ...
    logger.info("Static vs Dynamic Comparison Demo")
    logger.info("=" * 50)
    ...
    logger.info("User permissions: %s", [p.name for p in user_permissions])

    # Test with static prompts
    logger.info("Testing with static prompts...")
    ...
    logger.info("Static Approach Results:")
    if static_tool_calls:
        static_params = static_tool_calls[0].get("parameters", {})
        logger.info("  Tool calls: %d", len(static_tool_calls))
        logger.info("  Parameters: %s", static_params)
    else:
        logger.info("  No tool calls generated")

    logger.info("Testing with dynamic prompts...")
    ...
    dynamic_correct = any(
        correct_param in str(call.get("parameters", {})) for call in dynamic_tool_calls
    )
    static_correct = any(
        correct_param in str(call.get("parameters", {})) for call in static_tool_calls
    )

    logger.info("  Static approach used correct parameter: %s", "Yes" if static_correct else "No")
    logger.info("  Dynamic approach used correct parameter: %s", "Yes" if dynamic_correct else "No")

    if dynamic_correct and not static_correct:
        logger.info("  Dynamic prompts improved parameter selection!")
    elif static_correct and not dynamic_correct:
        logger.info("  Dynamic prompts performed worse than static")
    else:
        logger.info("  Both approaches performed similarly")


def demo_real_world_integration() -> None:
    """Show how this would integrate with real kfinance client."""

    logger.info("Real-World Integration Demo")
    logger.info("=" * 50)
    logger.info("This demonstrates how the dynamic prompt system would integrate")
    logger.info("with the actual kfinance client in production.")
    logger.info("Key integration points:")
    logger.info("  1. Client initialization loads cached embeddings")
    logger.info("  2. Query processing constructs dynamic prompts")
    logger.info("  3. LLM responses are enhanced with relevant examples")
    logger.info("  4. Tool parameter selection is more accurate")
    logger.info("For full integration, see the integration_example.py file.")

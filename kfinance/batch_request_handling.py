...
import threading
from typing import Any, Callable, Iterable, Protocol, Sized, Type, TypeVar, Hashable
                ...
                def create_prop_wrapper(method: property) -> Callable:
                    ...
                    def prop_wrapper(self: IterableKfinanceClass) -> Any:
                        ...
                        return process_tasks_in_thread_pool_executor(
                            api_client=self.kfinance_api_client,
                            tasks=[
                                Task(func=method.fget, args=(obj,), result_key=obj)
                                for obj in self
                            ]
                        )
                ...
                def create_cached_prop_wrapper(method: cached_property) -> cached_property:
                    ...
                    def cached_prop_wrapper(self: IterableKfinanceClass) -> Any:
                        return process_tasks_in_thread_pool_executor(
                            api_client=self.kfinance_api_client,
                            tasks=[
                                Task(func=method.func, args=(obj,), result_key=obj)
                                for obj in self
                            ]
                        )
...
def process_tasks_in_thread_pool_executor(
    ...
    with api_client.batch_request_header(batch_size=len(tasks)):
        for task in tasks:
            # Acquire throttle before submitting the task
            throttle.acquire()
            future = api_client.thread_pool.submit(

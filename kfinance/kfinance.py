...
class Tickers(set):
    ...
    def intersection(self, *s: Iterable[Any]) -> Tickers:
        ...
        self_triples = {t._fetch_id_triple for t in self}
        set_triples = []

        for ticker_set in s:
            set_triples.append({t._fetch_id_triple for t in ticker_set})
...
class Client:
    ...
    def get_tools(self, tool_mode: ToolMode) -> dict[str, Callable]:
        ...
        """
        return {
            t.name: t.run_without_langchain for t in self.get_langchain_tools(tool_mode=tool_mode)
        }
    ...
    def get_gemini_tool_descriptions(self, tool_mode: ToolMode) -> gapic.Tool:
        ...
        openai_tool_descriptions = [
            ...
            for t in self.get_langchain_tools(tool_mode=tool_mode)
        ]
    ...
    def get_openai_tool_descriptions(self, tool_mode: ToolMode) -> list[dict[str, Any]]:
        """Return tool descriptions for gemini"""
        openai_tool_descriptions = [
            convert_to_openai_tool(t) for t in self.get_langchain_tools(tool_mode=tool_mode)
        ]
